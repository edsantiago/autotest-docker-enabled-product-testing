...
class Variable(ActionBase):
    ...
    def init(self, name, value=None, from_env=None, remove=False, **dargs):
        """
        ...
        :param str from_env: Set value from environment variable with
                             this name, error if empty or unset.
        :param bool remove: Guarantee that variable ``name`` does not exist.
        ...
        if value and from_env or value and remove or from_env and remove:
            self.yamlerr('initializing',
                         'only one of value(%s), from_env(%s), or '
                         'remove(%s) may be set for %s'
                         % (value, from_env, remove, self.name))
        self._value = value
    ...
    def action(self):
        ...
        """
        if self._remove:
            del self.global_vars[self.name]
        elif self._from_env:
            ...
            self.global_vars[self.name] = self._value
        else:
